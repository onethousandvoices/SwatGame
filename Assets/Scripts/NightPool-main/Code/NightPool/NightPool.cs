...
namespace NTC.Global.Pool
{
    public static class NightPool
    {
        ...
        public static T Spawn<T>(T component, Vector3 position = default(Vector3), Quaternion rotation = default(Quaternion)) 
            where T : Component
        {
            return 
        ...
        public static T Spawn<T>(T component, Transform parent, Quaternion rotation = default(Quaternion), 
        ...
        {
            GameObject poolParent = new GameObject($"[NightPool] {prefab.name}");
        ...
        private static void CheckForSpawnEvents(GameObject toCheck)
        {
            ...
            for (int i = 0; i < ItemEventComponents.Count; i++)
            {
